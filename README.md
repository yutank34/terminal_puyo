# Terminal Puyo - Falling Block Puzzle Game

ターミナルで遊べる落ち物パズルゲームです。4つ以上の同じ色のブロックを繋げて消し、ゲームオーバーになるまでハイスコアを目指す一人用の耐久モードです。

SEGAのぷよぷよシリーズにインスパイアされた作品です。

## 機能

- ✅ 基本的なパズルゲームロジック（落下、移動、回転、消去、連鎖）
- ✅ **4色/5色モード選択**（起動時に選択可能）
- ✅ **壁キック・床キック機能**（端での回転時に自動調整）
- ✅ **設置猶予（32フレーム）** - クラシックな落ち物パズルゲームの仕様を参考
...
1. 上から2つ1組の色付きブロックが落ちてきます
2. 同じ色のブロックを4つ以上つなげると消えます
3. ブロックが消えた後、さらにつながると「連鎖」が発生し、高得点が狙えます
4. 10回消去するごとにレベルアップし、ブロックの落下速度が速くなります
5. フィールドの上までブロックが積み上がるとゲームオーバーです
...
### 操作のコツ
- **設置猶予**: ブロックが地面に着いても約0.5秒（32フレーム）は移動・回転可能です
...
1. ブロックペアが着地して固定
2. 重力を適用（落下）
3. 4つ以上つながったブロックを検出・消去
4. 画面に「N CHAIN!」と表示
5. 再び重力を適用
6. さらにつながりができたら3に戻る（連鎖継続）
7. つながりがなくなったら次のブロックペアを出現

### クラシックな落ち物パズルゲームの仕様を参考
